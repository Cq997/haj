    ...
    const medicalItemsData = [
      "جهاز لوكس","جهاز صدمات خارجي AED","جهاز تنفس صناعي","جهاز لايف باك","جهاز ميندري","جهاز قياس العلامات الحيوية",
...
• ${(row["ملاحظات عامة عن المناوبة"] || 'لايوجد').split('\n').filter(Boolean).join('\n• ')}`;
    ...
    function buildMedicalCheckTable(){
      ...
      medicalItemsData.forEach((item,index)=>{
        ...
        row.innerHTML = `
          <td>${item}</td>
          <td>
            <select id="medical_status_${index}">
              <option value="متوفر">متوفر</option>
              <option value="غير متوفر">غير متوفر</option>
            </select>
          </td>
          <td><input type="number" id="medical_count_${index}" min="0" value="0" class="compact-number"></td>
          <td><input type="text" id="medical_note_${index}" placeholder="ملاحظات"></td>
        `;
    ...
    async function sendPersonalCovenant(){
      ...
      for (let i = 0; i < tableBody.rows.length; i++) {
        ...
        const itemName = row.cells[0].querySelector('input').value.trim();
    ...
    function clearPersonalCovenantForm() {
      ...
      hideAlert("personalCovenantAlert");
          ...
          <td><input type="number" id="stock_current_${index}" min="0" value="0" class="compact-number"></td>
        `;
...
function updateDashboard(data){
  // KPIs
  const total = 148; // Fixed total points
  ...
  document.getElementById('kpi_total').textContent = 148;
  ...
  document.getElementById('dashLastUpdate').innerHTML = `
    ...
    <div style="font-size:.78rem;color:var(--muted);margin-top:4px">
      ${new Date().toLocaleString('ar-SA')} | إجمالي 148 نقطة   </div>
  `;
}
...
async function loadLiveDeploymentData(){
  try {
    ...
    if(data && data.length > 0){
      liveDeploymentData = data.map(d => ({
        id: d["الرمز"] || d["اسم النقطة"] || '',
        sector: d["القطاع"] || '',
        type: d["نوع الآلية"] || d["الالية المشغلة بالنقطة"] || '',
        teams: parseInt(d["عدد الفرق الفعلي"] || d["عدد الفرق بالنقطة"] || 1),
        lat: parseFloat(d["Latitude"] || 0),
        lng: parseFloat(d["Longitude"] || 0),
        status: d["الحالة"] || 'جاهز',
        shift: d["مناوبة النقطة"] || d["العرض الزمني"] || 'الصباحية'
      })).filter(d => d.lat && d.lng);
    }
  } catch(e) {
    console.log('استخدام البيانات الثابتة:', e);
  }
  updateDashboard(liveDeploymentData);
