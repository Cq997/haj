    ...
    // ربط أحداث النقر عند تحميل الصفحة
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.nav li[data-target]').forEach(item => {
          item.addEventListener('click', function() {
            const target = this.getAttribute('data-target');
            if (target && window.activatePanel) {
              window.activatePanel(target);
            }
          });
        });
      });
...
function updateDashboard(data){
  // KPIs
  const totalPoints = 148; // Fixed total points as requested
  ...
  const totalTeams = ready + supported + oos + broken;

  document.getElementById('kpi_total').textContent = totalPoints;
  document.getElementById('kpi_teams').textContent = totalTeams;
  ...
  Object.entries(types).sort((a,b) => b[1]-a[1]).forEach(([t,c]) => {
    typeBody.innerHTML += `<tr>
      ...
      <td>${c}</td>
      <td>${totalTeams ? Math.round(c/totalTeams*100) : 0}%</td>
    </tr>`;
  });
  ...
  chartEl.innerHTML = '';
  statuses.forEach(s => {
    const pct = totalTeams ? Math.round(s.count/totalTeams*100) : 0;
    chartEl.innerHTML += `
...
async function loadLiveDeploymentData(){
  try {
    ...
    if(data && data.length > 0){
      // فرز البيانات حسب التاريخ والوقت تنازلياً لضمان أخذ أحدث إدخال
      const sortedData = [...data].sort((a, b) => {
        const dateA = new Date(a["التاريخ"] + " " + (a["الوقت"] || "00:00"));
        const dateB = new Date(b["التاريخ"] + " " + (b["الوقت"] || "00:00"));
        return dateB - dateA;
      });
      ...
      sortedData.forEach(livePt => {
        ...
        if (liveId && !latestStatusMap.has(liveId)) {
          latestStatusMap.set(liveId, livePt);
        }
      });

      // تحديث البيانات المدمجة بأحدث الحالات
      latestStatusMap.forEach((livePt, liveId) => {
        const index = mergedData.findIndex(p => String(p.id).trim() === liveId);
        if(index !== -1){
          ...
          mergedData.push({
            ...
            lastUpdate: livePt["التاريخ"] + " " + (livePt["الوقت"] || "")
          });
        }
      });
    }
